...
public class RegexUtil {
     ...
     * @return
     */
    public static List<String> getSrcFromImgTag(String html) {
    	Pattern pt = Pattern.compile("(<img[^>]+src\\s*=\\s*['\"]([^'\"]+)['\"][^>]*>)");
    	Matcher match = pt.matcher(html);
    	List<String> list = new ArrayList<>();
    	while(match.find()) {
			String src = match.group(2);
			if(StrUtils.isNotEmpty(src)) {
				list.add(src);
			}
		}
    	return list;
     ...
     * @return
     */
    public static List<String> getSrcFromVideoTag(String html) {
    	Pattern pt = Pattern.compile("(<video[^>]+src\\s*=\\s*['\"]([^'\"]+)['\"][^>]*>)");
    	Matcher match = pt.matcher(html);
    	List<String> list = new ArrayList<>();
    	while(match.find()) {
			String src = match.group(2);
